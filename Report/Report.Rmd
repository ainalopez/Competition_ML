...
* **K-nearest neighbors:**   
We implemented a k-NN algorithm, trying both a function written by one of us, and an R package installed from CRAN. In both cases, the k-NN performed poorly. In terms of predictions, it's accuracy was lower compared to the Random Forest, and the k-NN algorithm was very expensive to compute. We ran the algorithm optimizing for different values of k, measuring distance using the Euclidean norm, yet the algorithm could not outperform the Random Forest. 
